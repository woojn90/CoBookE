package com.android.woojn.coursebookmarkapplication;

/**
 * Created by wjn on 2017-02-17.
 */

public class Constants {
    public static final String FIELD_NAME_ID = "id";
    public static final String FIELD_NAME_TITLE = "title";
    public static final String FIELD_NAME_FAVORITE = "favorite";
    public static final String KEY_COURSE_ID = "courseId";
    public static final String KEY_SECTION_ID = "sectionId";
    public static final String KEY_FOLDER_ID = "folderId";
    public static final String KEY_STRING_URL = "stringUrl";
    public static final String KEY_TITLE = "title";
    public static final String KEY_FILE_NAME = "file_name";

    public static final String FILE_NAME_LICENSE_APACHE = "license_apache.txt";
    public static final String FILE_NAME_LICENSE_BUTTERKNIFE = "license_butterknife.txt";
    public static final String FILE_NAME_LICENSE_GLIDE = "license_glide.txt";
    public static final String FILE_NAME_LICENSE_JSOUP = "license_jsoup.txt";
    public static final String FILE_NAME_LICENSE_ONBOARDER = "license_onboarder.txt";
    public static final String FILE_NAME_LICENSE_REALM = "license_realm.txt";

    public static final int DEFAULT_VIEW_ID = 0;
    public static final int DEFAULT_SECTION_ID = 0;
    public static final int DEFAULT_FOLDER_ID = 0;
    public static final int PAGE_COURSE = 0;
    public static final int PAGE_ITEM = PAGE_COURSE + 1;
}
